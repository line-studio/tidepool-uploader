...
import createTidepoolClient from '@line-studio/tidepool-platform-client';
...
api.user.libreViewLogin = (cb) => {
  tidepool.getLibreViewAuthTicket((err, authTicket) => {
    if (err) throw err
...
api.user.uploadScrapedData = (targetUserId, data, cb) => {
  tidepool.uploadScrapedData(targetUserId, data, (err, res) => {
    if (err) throw err
...
api.upload.toPlatform = (data, sessionInfo, progress, groupId, cb, uploadType = 'jellyfish') => {
  ...
  const post_and_progress = (data, callback) => {
    progress(((nblocks += nblocks) * 100.0) / blocks.length);
